...
constexpr uint32_t NUM_PORT = 1; // sum switch egress port(used) 
const size_t leafswitch_portnum = 8; // leaf switch connect 6 server and 2 spine
const size_t spineswitch_portnum = 4; // spine connect 4 leaf switch 

struct sEcnRlEnv {
    uint32_t egressqlen; // egress qlen list
    double linkrate; // link rate list
    double ecnlinkrate; // ecn marked rate list
    uint32_t ecnmin; 
    uint32_t ecnmax;
    double ecnpmax;
    uint8_t envType; // 
...
public:
	...
	std::vector<Ptr<SwitchNode>>  m_globalSwList;
...
EcnRlEnv::DoScheduleNextStateRead(){
	...
	//Set the interval for calculating the ecn tagging rate
  	env->envType = 1;
  	env->simTime_us = Simulator::Now().GetNanoSeconds();

	const size_t totalPorts = 1 ;
    ...
    for (size_t i = 0; i < m_globalSwList.size() && globalPortIndex < totalPorts; ++i) {
		...
		env->egressqlen = switchNode->average_queue_port_list[25];
		env->linkrate= switchNode->txRate[25];
		env->ecnlinkrate= switchNode->ecn_txRate[25];
		env->ecnmin= switchNode->min_thresh[25];
		env->ecnmax = switchNode->max_thresh[25];
		env->ecnpmax = switchNode->pmax_thresh[25];
		// std::cout<<"ECN reading: min"<< switchNode->min_thresh[25]<<"    max"<<switchNode->max_thresh[1]<<"	 p"
		// <<switchNode->pmax_thresh[25]<<std::endl;
		
		// outFile << "Switch Node " << i << " Data:\n";
		// outFile << "  Egress Queue Length: " << switchNode->average_queue_port_list[25] << "\n";
		// outFile << "  Link Rate: " << switchNode->txRate[25] << "\n";
		// outFile << "  ECN Link Rate: " << switchNode->ecn_txRate[25] << "\n";
		// outFile << "  ECN Min Threshold: " << switchNode->min_thresh[25] << "\n";
		// outFile << "  ECN Max Threshold: " << switchNode->max_thresh[25] << "\n";
		// outFile << "  ECN Pmax Threshold: " << switchNode->pmax_thresh[25] << "\n";
		// outFile << "------------------------------\n";
		
		//std::cout << "Switch device: " <<i<<'\t'<< switchNode->GetNDevices() << '\t'<< switchNode->GetId()<< std::endl;
         // 获取当前交换机的端口数量
		/*
		if (globalPortIndex <= 31){
			// get information from leaf switch egresss port
    ...
    SetCompleted();
  	...
  	{
		m_new_min = act->newecnmin;
		m_new_max = act->newecnmax;
		m_new_pmax = act->newecnpmax;
		//std::cout<<m_new_min<<"   "<<m_new_max<<"   "<<"   "<<m_new_pmax<<std::endl;
	}
  	GetCompleted();
  	

	// 遍历每个交换机
	Ptr<SwitchNode> Switch = m_globalSwList[0];
	Switch->m_mmu->ConfigEcn(25, m_new_min, 
									m_new_max, 
									m_new_pmax);
	//std::cout<<m_new_min<<"\t"<<m_new_max<<"\t"<<m_new_pmax<<'\n';
	
	/*
	for (size_t switchIdx = 0; switchIdx < 6; ++switchIdx) {
		...
		if(switchIdx <4){
			...
			size_t startGlobalPortIndex = 32 + (switchIdx-4) * 4;
			for (size_t localPortIdx = 0; localPortIdx < 4; ++localPortIdx) 
			{
				size_t globalPortIndex = startGlobalPortIndex + localPortIdx;
				//std::cout << Switch->GetId() << '\t' <<Switch->GetNDevices() << '\t'<<globalPortIndex << ' '<< localPortIdx +1 <<std::endl;
				// 使用全局端口索引从总数组中获取值
	...
	*/
	//for test: the ai training time
	// std::cout<<"finish interaction. time is "<<Simulator::Now().GetMicroSeconds()<<"ms"<<std::endl;
...
{	
		...
		int memblock_key = 3230;        ///< memory block key, need to keep the same in the python script
