...
class PPO:
    ...
    def __init__(self,
                 ...
                 Kmax,
                 Kmin,
                 model_file,replaybuffer,batch_size,train = 'T'):
    ...
    def load(self,policy_name,value_name):
        if os.path.exists(self.model_file):
            policy_dict = torch.load(self.model_file+policy_name,weights_only=True)
            self.policy.load_state_dict(policy_dict)
            print("agent load actor_net successfully")
            value_dict = torch.load(self.model_file+value_name,weights_only=True)
    ...
    def save(self,iter):
        ...
        actor_name = f'Model/model_trainh/actor_'+str(iter)+'.pt'
        critic_name = f'Model/model_trainh/critic_'+str(iter)+'.pt'
        # 创建目录（如果不存在）
